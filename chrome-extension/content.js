...
function resolveCartClick(target, event, siteKey) {
  ...
  for (const start of roots) {
    ...
    for (let depth = 0; depth < 18 && node; depth++, node = node.parentElement) {
      ...
      if (!["button", "a", "input"].includes(tag) && node.getAttribute("role") !== "button") continue;
      const text = (node.innerText || node.textContent || "").replace(/\s+/g, " ").trim();
      if (text && /sepete\s*ekle|şimdi\s*al|hemen\s*al|buy\s*now|add\s*to\s*(cart|basket)/i.test(text) && text.length < 120 && isVisible(node)) return node;
    }
  }
  return null;
}
    ...
    <div class="fg-glass">
      ...
      ${(alternatives && alternatives.length > 0 && !isLoading) ? `
        <div style="margin:14px 0;background:rgba(6,182,212,0.06);border:1px solid rgba(6,182,212,0.15);border-radius:12px;padding:14px;">
          <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:#67e8f9;margin-bottom:10px;display:flex;align-items:center;gap:6px;">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;">
            ${alternatives.map(alt => `
              <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:rgba(255,255,255,0.04);border-radius:8px;border:1px solid rgba(255,255,255,0.06);">
                <div style="font-size:12px;font-weight:600;color:rgba(255,255,255,0.85);">${alt.site || alt.platform || 'Site'}</div>
                <div style="display:flex;align-items:center;gap:8px;">
                  <span style="font-size:13px;font-weight:700;color:#6ee7b7;">${typeof alt.price === 'number' ? alt.price.toLocaleString('tr-TR') : alt.price} ₺</span>
                  ${alt.url ? `<a href="${alt.url}" target="_blank" style="font-size:11px;padding:3px 8px;background:rgba(6,182,212,0.15);border:1px solid rgba(6,182,212,0.3);border-radius:5px;color:#67e8f9;text-decoration:none;font-weight:600;">Git →</a>` : ''}
                </div>
              </div>`).join('')}
          </div>
          <div style="margin-top:6px;font-size:10px;color:rgba(255,255,255,0.3);">⚠️ Fiyatlar yaklaşıktır, tıklayarak doğrulayın.</div>
        </div>` : ''}
  ...
  if (!isLoading) {
    ...
    if (okBtn) {
      okBtn.onclick = () => {
        closeOverlay();
        if (decision === 'AL') {
          const el = _finguardPendingCartEl;
          if (el) {
            _finguardBypass = true;
            simulateRealClick(el);
            setTimeout(() => { _finguardBypass = false; }, 500);
          }
          _finguardPendingCartEl = null;
        }
      };
    }
    const forceBtn = document.getElementById("fg-force-buy");
    if (forceBtn) {
      const freshBtn = forceBtn.cloneNode(true);
      forceBtn.replaceWith(freshBtn);
      document.getElementById("fg-force-buy").onclick = () => {
        closeOverlay();
        const el = _finguardPendingCartEl;
        if (el) {
          _finguardBypass = true;
